...
class FormInputTest extends TestCase
{
    ...
    public function test_setMaxLength1() : void
    {
        $oFG = $this->createFG(true);
        $oFL = $oFG->add(new FormLine('testline'));
        $oInput = $oFL->add(new FormInput('strInput1', '20px', 0, 100));
        $strHTML = $oInput->getHTML();
        $this->assertNotFalse(strpos($strHTML, 'maxlength="100"'));
    }

    public function test_setMaxLength2() : void
    {
        $oFG = $this->createFG(true);
        $oFL = $oFG->add(new FormLine('testline'));
        $oInput = $oFL->add(new FormInput('strInput1', '20px'));
        $oInput->setMaxLength(100);
        $strHTML = $oInput->getHTML();
        $this->assertNotFalse(strpos($strHTML, 'maxlength="100"'));
    }
    ...
    public function test_fromXML() : void
    {
        $oFG = $this->createXMLFG(true);
        $oFG->loadXML(__DIR__ . DIRECTORY_SEPARATOR . 'testdata/FormInput.xml');
        $strHTML = $oFG->getForm();
        $this->assertNotFalse(strpos($strHTML, 'name="strLastname"'));
    }
}
